...
/**
 * Content optimization scans, powered by the operator's DataForSEO key (the
 * same BYO key the rest of the app uses — no separate provider connection).
 * Both tools no-op with a clear message when the module is disabled or the
 * DataForSEO key is absent.
 */

const NOT_CONFIGURED_TEXT =
  "Content Optimization needs a DataForSEO API key. Set DATAFORSEO_API_KEY and restart to enable content optimization scans.";
...
async function unavailableText(): Promise<string | null> {
  const status = await ContentOptimizationService.connectionStatus();
  if (!status.enabled) return MODULE_DISABLED_TEXT;
  if (!status.configured) return NOT_CONFIGURED_TEXT;
  return null;
}
...
export const runContentScanTool = {
  ...
  config: {
    ...
    description:
      "Start a content optimization scan for a URL and keyword: entity coverage versus the live SERP cohort, structural benchmarks against the page-1 average, competitor term gaps, and content suggestions. Asynchronous (usually 30 seconds to 3 minutes); poll get_content_scan for the result. Uses DataForSEO on-page credits.",
    inputSchema: runInputSchema,
    outputSchema: z
      .object({
        // A disabled or unconfigured module answers with the reason and starts
  ...
  handler: withMcpProjectAuth(async (args: RunArgs, context) => {
    ...
    await captureServerEvent({
      ...
      properties: { projectId: args.projectId, provider: "dataforseo" },
    });
    return mcpResponse({
...
export const getContentScanTool = {
  ...
  handler: withMcpProjectAuth(async (args: GetArgs, context) => {
    ...
    if (view.status === "failed") {
      return mcpResponse({
        text: `Scan failed: ${view.error ?? "no error details."}`,
    ...
    if (view.status !== "completed") {
      ...
      return mcpResponse({
        text: `Scan is running${pct}. Poll again in a few seconds.`,
        structuredContent: { status: view.status, progress: view.progress },
        meta,
      });
    }
    ...
    const digest = {
      ...
      poweredBy: report.poweredBy ?? "DataForSEO + AI",
    };
